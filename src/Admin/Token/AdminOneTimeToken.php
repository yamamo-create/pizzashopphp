...
class AdminOneTimeToken
{
    public static function generate(string $key): string
    {
        $token = bin2hex(random_bytes(32));
        $_SESSION['admin']['one_time_token'][$key] = $token;
        return $token;
    }
    ...
    {
        return !isset($_SESSION['admin']['one_time_token'][$key]);
    }
    ...
    private static function validate(string $key, ?string $postToken): bool
    {
        ...
        $sessionToken = $_SESSION['admin']['one_time_token'][$key] ?? null;
    ...
    {
        unset($_SESSION['admin']['one_time_token'][$key]);
    }
}
