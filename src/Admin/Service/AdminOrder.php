...
class AdminOrder
{
    ...
    // ----- admin order enter.php -----
    public function getCurrentOrder(): array
    {
        $result = [];

        $orderData = $this->orderRepository->findOrderPlusCustomer();
        $this->orderRepositoryValidator->validateOrderRepositoryFindOrderPlusCustomer($orderData);

        $ordeitemrData = $this->orderRepository->findOrderPlusOrderitem();
        $this->orderRepositoryValidator->validateOrderRepositoryFindOrderPlusOrderitem($ordeitemrData);
    ...
    {
        $this->adminPostValidator->validatePostAdminOrderCheck($post);
    }
