...
try {
    $orderData = $adminSales->getCompleteOrder();
} catch (Throwable $e) {
    RedirectPage::adminErrPage($e);
}

$admin_csrf_token = AdminCsrf::ensure();
$admin_sales_one_token = AdminOneTimeToken::generate('admin_sales_one_token');
?>
<!DOCTYPE html>
<html lang="ja">
                ...
                <section class="bgcolor">
                    <article>
                        <form method="post" action="check_done.php">
                            ...
                            <input type="hidden" name="admin_sales_one_token" value="<?php echo h($admin_sales_one_token); ?>">
                            <button class="button" type="submit" name="choice" value="received">注文に戻す</button>
                            <button class="button" type="submit" name="choice" value="canceled">キャンセル一覧に入れる</button>
                            <br><br>
