...
if (!AdminOneTimeToken::validateAndDestroy(
    'admin_staff_delete_one_token',
    $_POST['admin_staff_delete_one_token'] ?? null
)) {
    RedirectPage::adminErrPage(
...
$id = $_SESSION['admin']['form']['delete']['id'] ?? null;
$email = $_SESSION['admin']['form']['delete']['email'] ?? null;

unset($_SESSION['admin']['form']['delete']);

try {
    $sessionValidator->validateDeleteIdEmail($id, $email);
} catch (Throwable $e) {
...
// ----- スタッフ削除 -----
try {
    $adminManageStaff->deleteStaff($id, $email);
} catch (Throwable $e) {
    RedirectPage::adminErrPage($e);
}
