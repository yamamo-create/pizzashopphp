...
if (!AdminOneTimeToken::validateAndDestroy(
    'admin_product_update_one_token',
    $_POST['admin_product_update_one_token'] ?? null
)) {
    RedirectPage::adminErrPage(
...
$adminManageProduct->validateProductNotUsedUpdate($UseProductIds, $choiceId);
...
$newProductData = $adminManageProduct->trimPostUpdate($_POST);
$adminManageProduct->validatePostUpdate($newProductData, $_FILES);
...
try {
    $adminManageProduct->validateImageFileUpdate($_FILES);
} catch (Throwable $e) {
    RedirectPage::adminErrPage($e);
}
...
// ----- 古い画像ファイルを移動させるための処理の用意 -----
try {
    $oldProductData = $adminManageProduct->getAdminProductData($choiceId);
} catch (Throwable $e) {
    RedirectPage::adminErrPage($e);
}

$oldImagename = $oldProductData['imagename'] ?? '';

// ----- ここから登録 -----
try {
    $adminManageProduct->updateProduct($choiceId, $newProductData, $_FILES, $oldImagename);
} catch (Throwable $e) {
    RedirectPage::adminErrPage($e);
}
